...
| D28 | **Text-safe "deep" colour tokens** (`text-brand-deep`, `text-mint-deep`, `text-chili-deep`, …) for any status/brand-coloured *text*; the bright tokens stay for fills and icons. | axe WCAG 2 AA scan: saffron/mint/red text on tinted backgrounds failed 4.5:1. All 15 screens now pass in light + dark. |
| D29 | **Hand-written service worker** (`public/sw.js`): network-first pages with offline fallback, cache-first hashed assets, **never** caches `/api/*`. No PWA plugin. | Live clocks and orders must never be served stale; one small file is easy to audit and works on any host. |
| D30 | **Analytics charts are hand-rolled SVG** (no chart library) with colour tokens validated for colour-blind separation and contrast; every chart has a table view. | Small bundle, on-brand, accessible. |
